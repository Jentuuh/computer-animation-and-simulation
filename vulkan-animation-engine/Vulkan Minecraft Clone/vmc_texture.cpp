...
namespace vae {
    ...
    VmcTexture::VmcTexture(VmcDevice& device, const char* imagePath, VmcTextureType textureType) : device{ device } 
    {
        switch (textureType)
        {
	...
	void VmcTexture::createTextureImage(const char * imagePath)
	{
    ...
    void VmcTexture::setupCubeMap(const char* imagePath, VkFormat format)
    {
        ktxResult result;
    ...
    void VmcTexture::createTextureImageView()
    {
        textureImageView = device.createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_VIEW_TYPE_2D, 1);
    }

    void VmcTexture::createTextureImageCubeMap(const char* imagePath)
    {
        int texWidth, texHeight, texChannels;
        stbi_uc* pixels = stbi_load(imagePath, &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    ...
    void VmcTexture::createTextureImageViewCubeMap()
    {
        textureImageView = device.createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_VIEW_TYPE_CUBE, 6);
    }


    void VmcTexture::createTextureSampler(VkSamplerAddressMode addressMode)
    {
        VkPhysicalDeviceProperties properties{};
        vkGetPhysicalDeviceProperties(device.getPhysicalDevice(), &properties);
    ...
    VkDescriptorImageInfo VmcTexture::descriptorInfo()
    {
